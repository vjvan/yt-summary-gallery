...
7. **外部翻譯流程（09-08 上午）**：`docs/external-translation-spec.md` 是給 Claude／Codex／人工翻整片的規範，`scripts/translation-import.ts` 做 export（帶前後文分批）／check（句數、句序、空句、殘留英文、數字、保留詞、大陸用語、長度離群）／load（走語意校訂的候選與套用路徑寫回，可整批還原）。首例：GtxJdBTXKwE 1123 句由 8 個 Sonnet 5 子代理平行翻（約 9 分鐘），驗證 0 錯誤 43 警告（37 個是合理保留的英文），`load --apply` 套用 1119 句、SRT／VTT 重寫，模型標 `external:claude-sonnet-5`。**尚未回寫 Chrome 擴充的即時字幕快取**（`data/watch*.db`），YouTube 原站看仍會逐句跑 7B，這是下一步。
